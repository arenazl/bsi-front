...
interface Props {
  getUsers: () => Promise<Contrato[]>; // por ahora se llama getUsers()
...
export default function ContratoManagement({ getUsers, createUser, updateUser, deleteUser, showAlert }: Props) {
  ...
  const fetchContratos = useCallback(async () => {
    try {
      ...
      showAlert('Contratos cargados correctamente', 'success');
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      try {
        if (isEditing) {
          await updateUser(currentContrato.id_contrato, currentContrato);
          showAlert('Contrato actualizado exitosamente', 'success');
        } else {
          await createUser(currentContrato);
        ...
        showAlert('Error al guardar el contrato', 'error');
      }
    }
  };
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      showAlert('Error al eliminar el contrato', 'error');
    }
  };
